...
from src.exp.bboxes import highlight_img

ex = get_skira_exp("test_highlight_img")
...
def test_crop(video_directory, video, tracks_file):
    ...
    output_video = f'/tmp/{time()}_test_highlight.png'
    ...
    ex.add_artifact(output_video, video.replace('/', '_').replace(' ', '_') + '.png')
